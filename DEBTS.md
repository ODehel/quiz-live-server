...
- **Validateurs instanciés en dur dans la route** (DIP) — `new Uuidv7Validator()` dans le `POST`, `new UuidFormatValidator()` dans le `GET` : deux occurrences. Injection via `QuestionRouteConfiguration` à la troisième (touche `question-route.test`, `server.test` ×2, `index.ts`).
...
- **Codes d'erreur absents** — CA-6 `VALIDATION_ERROR` (`400` nu), CA-8 `INVALID_UUID` (`400` nu), body `UNAUTHORIZED` du `401`.
